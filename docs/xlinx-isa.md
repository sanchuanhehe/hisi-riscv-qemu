...
## 掩膜 ROM 调用拦截（让主应用 ws63-liteos-app 跑起来）

`ws63-liteos-app` 调用 53 个固化在硅片掩膜 ROM（0x109000–0x14C000）的函数（`vsnprintf_s`/`memcpy_s`/
SFC/pin/timer/systick…，ABI 见 `rom_config/acore/acore.sym`），SDK 不提供 ROM 二进制。我们在
`riscv_cpu_do_interrupt`（补丁内）拦截：app 跳进 ROM 区 → 空 ROM 触发非法指令 → 用**宿主 C 模拟**该函数
并返回 `ra`。多数桩为「返回 0（成功）」；`mem*_s` 实做、`*printf_s` 族委托宿主 libc（故 app 的格式化日志能打出）、
时间 getter 返回计数、funcs-getter 返回预填 trampoline 的可写 scratch vtable。

**结果**：主应用启动 LiteOS、打印格式化日志、创建全部 14 个子系统任务（app/log/bt/wifi/radar/at…）、
**进入调度器**（`cpu 0 entering scheduler`），见 [`logs/ws63-liteos-app-boot.txt`](logs/ws63-liteos-app-boot.txt)：
```
...
```
随后在 `bt` 任务里崩（near-null load）——因 BT/NV/efuse 等**ROM 数据 + 硬件状态**未建模。

## 已知边界（非 ISA 问题）

- **掩膜 ROM 数据墙**：ROM 函数桩能让 app 跑到调度器，但 ROM 里的**数据结构**（vtable 内容、配置、NV/efuse、
  BT/WiFi 标定）无 dump，深层子系统初始化（bt/wifi）会因 near-null/缺数据而崩。整应用跑到底需真实 ROM dump。
