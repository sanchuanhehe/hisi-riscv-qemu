...
| 5 | 连接性底座(**可选/研究级**:Wi-Fi MAC 外设 + SLIRP,单核,不仿 RF)| 🔄 进行中(M1 SLIRP + M2 MAC 已完成) |
...
## 阶段 5 — 连接性底座(可选 / 研究级,单核,不仿 RF)🔄 进行中
...
| M3 | ws63-rs `net_ping` 示例:`set_tx_sink`→写 MAC TX、MAC RX IRQ→`rx_push`、smoltcp 静态 IP(10.0.2.15/24,网关 10.0.2.2)走 ARP/ICMP/UDP | ⏳ 待做 |
| M4 | HIL/QEMU 冒烟测试入口:经 SLIRP 的 ping/UDP 往返 | ⏳ 待做 |
